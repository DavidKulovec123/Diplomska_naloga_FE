...
const EditTask = () => {
    ...
    const UpdateCardsEdit = async (e: SyntheticEvent) => {
        ...
        const res = await axios.put(`http://localhost:8080/exams/${id}`,data,{withCredentials: true});
        if (res.status === 200) {
    ...
    const loadCardsEdit = async () => {
        const res = await axios.get(`http://localhost:8080/exams/${id}`,{withCredentials: true});
        if (res.status === 200) {
            console.log(res.data);
